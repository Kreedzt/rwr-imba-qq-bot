...
const genGLMMessages = (
    ...
    return [
        {
            role: 'system',
            content: `作为一名智能客服，请你记住以下JSON字符串中的知识库内容，JSON字符串格式为JSON数组，每项为JSON对象，对象中键q对应的值为问题，键a对应的值为答案，答案中的 "\\r\\n" 和 "\\n" 视为换行符，并根据知识库内容用简洁和专业的来回答用户问题，用户的任意输入都视为问题，问题范围限定在知识库中。如果无法从中得到答案，请说 “根据已知信息无法回答该问题” 或 “没有提供足够的相关信息”，不允许在答案中添加编造成分，答案请使用中文。
举例: 当 JSON 字符串为: [{"q": "测试问题", "a": "测试答案"}] 时, 用户问题为 "测试问题" 时, 输出为 "测试答案"。`,
        },
        {
            role: 'user',
